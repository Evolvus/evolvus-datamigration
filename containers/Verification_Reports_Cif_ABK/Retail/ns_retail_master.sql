...
set pages 0
set trimspool on
spool $MIG_PATH/output/reports/kw/cif/ns_retail_master.dat
select
'LEG_BRANCH'||'|'||
'LEG_CUST_NUMBER'||'|'||
'FINACLE_SOL_ID'||'|'||
...
to_char(LEG_SEGMENT)||'|'||
to_char(FIN_SEGMENT)||'|'||
to_char(LEG_SUBSEGMENT)||'|'||
to_char(FIN_SUBSEGMENT)||'|'||
...
from rep_master where sort1='NON_STAFF';
exit; 
